...
## Overview of FastAPI API Layer

The QRiskLab API is built using FastAPI, which provides a modern, fast (high-performance) web framework for building APIs with Python 3.7+ based on standard Python type hints. FastAPI automatically validates request and response data using Pydantic V2 models, ensuring type safety and data integrity.
...
The API includes the following main areas:

- **Pricing Endpoints**: For European call option pricing and batch pricing.
- **Risk Endpoints**: For calculating risk metrics such as VaR and CVaR.
- **Quantum Endpoints**: For executing quantum algorithms and operations.

Additionally, request and response validation uses Pydantic V2 models defined in `qrisklab/api/models.py`.
...
OpenAPI documentation is automatically generated by FastAPI and is available when the app is running.
